...
(defn flux-image-command
  ([request]
   (flux-image-command request :flux-pro))
  ([request model]
   (let [text (get-in request [:form-params "text"])
         channel-id (get-in request [:form-params "channel_id"])
         response-url (get-in request [:form-params "response_url"])
...
(defn ideogram-image-command [request]
  (flux-image-command request :ideogram))
